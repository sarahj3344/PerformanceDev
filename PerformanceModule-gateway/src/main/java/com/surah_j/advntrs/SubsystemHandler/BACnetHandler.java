...
public class BACnetHandler extends SubsystemBase {
    private final LoggerEx log = LogUtil.getLogger(getClass().getSimpleName());
    GatewayContext context = GatewayHook.context;

    public BACnetHandler(String subsystem) {
        super(subsystem);
    }
    ...
    public List<Map> getConnectionNames(PersistenceSession session) {
        ...
        try {
            results = session.rawQueryMaps(("SELECT DEVICENAME FROM " + recordsMap.get(subsystem)), true);
        } finally {
            session.close();
        }
    ...
    public JSONObject configureSettings(String connectionName) throws JSONException {
        ...
        try {
            ...
            if (subsystem != null && !subsystem.isEmpty()) {
                String queryString = "SELECT BINDPORT FROM " + recordsMap.get(subsystem);
                log.info(queryString);
                results = session.rawQueryMaps((queryString), true);
                settingsJson.put("Port", results.get(0).get("BINDPORT"));
                settingsJson.put("Filter", "udp and port " + results.get(0).get("PORT"));
            }
        } finally {
            session.close();
        }
