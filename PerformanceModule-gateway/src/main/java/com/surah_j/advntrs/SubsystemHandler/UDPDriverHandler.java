...
public class UDPDriverHandler extends SubsystemBase {
    private final LoggerEx log = LogUtil.getLogger(getClass().getSimpleName());
    GatewayContext context = GatewayHook.context;

    public UDPDriverHandler(String subsystem) {
        super(subsystem);
    }
    ...
    public JSONObject configureSettings(String connectionName) throws JSONException {
        ...
        try {
            ...
            if (subsystem != null && !subsystem.isEmpty()) {
                ...
                settingsJson.put("Filter", "host " + results.get(0).get("HOSTNAME") + " and port " + results.get(0).get("PORT") + "and udp");
            }
        } finally {
            session.close();
        }
